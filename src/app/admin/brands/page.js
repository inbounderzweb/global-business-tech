...
export default function BrandsPage() {
    ...
    const saveEdit = async () => {
        try {
            ...
            if (res.ok) {
                setBrands(brands.map((b) => (b.id === editingId ? { ...b, name: editName } : b)));
                setEditingId(null);
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="max-w-3xl">
            <div className="mb-12">
                <h1 className="text-3xl font-bold text-slate-900 mb-2">OEM Brands</h1>
                <p className="text-slate-500 font-medium">Manage the brands products can be assigned to.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="space-y-4">
                    ...
                    {brands.length > 0 ? brands.map((b) => (
                        <div
                            key={b.id}
                            className={`group flex items-center justify-between p-5 rounded-[24px] border border-slate-100 bg-white transition-all hover:shadow-lg ${editingId === b.id ? 'ring-2 ring-blue-500 border-transparent shadow-blue-100' : ''}`}
                        >
                            {editingId === b.id ? (
                                <input
                                    value={editName}
