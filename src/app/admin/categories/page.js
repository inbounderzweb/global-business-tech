...
export default function CategoriesPage() {
    ...
    const saveEdit = async () => {
        try {
            ...
            if (res.ok) {
                setCategories(categories.map(c => c.id === editingId ? { ...c, name: editName, description: editDescription } : c));
                setEditingId(null);
            }
        } catch (err) {
            console.error(err);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm h-fit sticky top-10">
                    ...
                    <form onSubmit={addCategory} className="space-y-4">
                        ...
                        <textarea
                            ...
                            rows={3}
                            className="w-full rounded-2xl border border-slate-200 bg-slate-50/50 px-5 py-4 text-sm font-medium text-slate-900 outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 resize-none"
                        />
                ...
                <div className="space-y-4">
                    ...
                    {categories.length > 0 ? categories.map((c) => (
                        <div
                            key={c.id}
                            className={`group p-5 rounded-[24px] border border-slate-100 bg-white transition-all hover:shadow-lg ${editingId === c.id ? 'ring-2 ring-blue-500 border-transparent shadow-blue-100' : ''}`}
                        >
                            {editingId === c.id ? (
                                <div className="space-y-3">
                                    <input
                                        ...
                                        autoFocus
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-2 outline-none font-bold text-slate-900 focus:border-blue-500"
                                    />
                                    <textarea
                                        ...
                                        rows={2}
                                        className="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-3 py-2 outline-none text-sm font-medium text-slate-700 focus:border-blue-500 resize-none"
                                    />
                                    <div className="flex gap-2">
                                        <button onClick={saveEdit} className="px-4 py-2 rounded-xl bg-slate-900 text-white text-xs font-bold hover:brightness-110 transition">Save</button>
                                        <button onClick={() => setEditingId(null)} className="px-4 py-2 rounded-xl bg-slate-100 text-slate-500 text-xs font-bold hover:bg-slate-200 transition">Cancel</button>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        {c.description && <p className="mt-1 text-xs text-slate-400 font-medium line-clamp-1">{c.description}</p>}
                                    </div>
